...
public class MemberInfoCommand implements ServerCommand {
	...
	public void onInfo(Member requester, Member u, GuildMessageChannel channel) {
		...
		StringBuilder description = new StringBuilder().append("**User: **").append(u.getAsMention()).append("\n")
				.append("**ClientId: **").append(u.getId()).append("\n").append("**TimeJoined: **")
				.append(u.getTimeJoined()).append("\n").append("**TimeCreated: **").append(u.getTimeCreated())
				.append("\n").append("**Nickname: **").append(u.getNickname()).append("\n").append("**Status: **")
				.append(u.getOnlineStatus()).append("\n").append("**Is Owner: **").append(u.isOwner()).append("\n")
				.append("**Permissions: **").append(u.getPermissions()).append("\n\n");

		description.append("**Roles: **\n")
				.append(u.getRoles().stream().map(Role::getAsMention).collect(Collectors.joining(" "))).append("\n");

		EmbedBuilder builder = EmbedUtils.getBuilderOf(Color.red, description, channel.getGuild().getIdLong());
