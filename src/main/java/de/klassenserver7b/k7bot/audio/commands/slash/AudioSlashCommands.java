...
public class AudioSlashCommands {
    ...
    private static boolean ensureConnected(SlashCommandInteraction event, Member caller) {
        ...
        if (memberVoiceState == null || !memberVoiceState.inAudioChannel()) {
            if (event.isAcknowledged()) {
                event.getHook().sendMessageEmbeds(EmbedUtils.getErrorEmbed("You must be in a voice channel to use this command!", event.getGuild().getIdLong()).build()).queue();
            } else {
                event.replyEmbeds(EmbedUtils.getErrorEmbed("You must be in a voice channel to use this command!", event.getGuild().getIdLong()).build()).queue();
            }
            return false;
        }
    ...
    public static class PlayCommand implements TopLevelSlashCommand {
        ...
        public SlashCommandData getCommandData() {
            return Commands.slash("play", "Replace current track and play")
        ...
        public void performSlashCommand(SlashCommandInteraction event) {
            ...
            if (m == null || !ensureConnected(event, m)) return;
            ...
            event.getHook().sendMessageEmbeds(EmbedUtils.getInfoEmbed("Loading track: " + query, guildId).build()).queue();
        }
    }
    ...
    public static class StopCommand implements TopLevelSlashCommand {
        ...
        public void performSlashCommand(SlashCommandInteraction event) {
            ...
            event.replyEmbeds(EmbedUtils.getSuccessEmbed("Stopped playback and left the channel.", guildId).build()).queue();
        }
    }
    ...
    public static class ForwardCommand implements TopLevelSlashCommand {
        ...
        @Override
        public SlashCommandData getCommandData() {
            return Commands.slash("forward", "Forward the current track")
                    .addOption(OptionType.INTEGER, "amount", "Amount in milliseconds to forward", true);
        }
    ...
    public static class BackCommand implements TopLevelSlashCommand {
        ...
        @Override
        public SlashCommandData getCommandData() {
            return Commands.slash("back", "Rewind the current track")
                    .addOption(OptionType.INTEGER, "amount", "Amount in milliseconds to rewind", true);
        }
    ...
    public static class VolumeCommand implements TopLevelSlashCommand {
        ...
        @Override
        public SlashCommandData getCommandData() {
            return Commands.slash("volume", "Set playback volume")
    ...
    public static class SeekCommand implements TopLevelSlashCommand {
        ...
        @Override
        public SlashCommandData getCommandData() {
            return Commands.slash("seek", "Seek to a specific position")
                    .addOption(OptionType.INTEGER, "position", "Position in milliseconds", true);
        }
    ...
    public static class LoopCommand implements TopLevelSlashCommand {
        ...
        public void performSlashCommand(SlashCommandInteraction event) {
            ...
            event.replyEmbeds(EmbedUtils.getSuccessEmbed("Looping is now " + (!repeating ? "enabled" : "disabled"), guildId).build()).queue();
        }
    }
    ...
    public static class ShuffleCommand implements TopLevelSlashCommand {
        ...
        public void performSlashCommand(SlashCommandInteraction event) {
            ...
            event.replyEmbeds(EmbedUtils.getBuilderOf(java.awt.Color.decode("#A537FD"), "Playlist shuffled", guildId).build()).queue();
        }
    }
    ...
    public static class EQCommand implements TopLevelSlashCommand {
        ...
        public SlashCommandData getCommandData() {
            return Commands.slash("eq", "Set equalizer preset")
                    .addOptions(new net.dv8tion.jda.api.interactions.commands.build.OptionData(OptionType.INTEGER, "preset", "The EQ preset to apply", true)
                            .addChoice("Off", 0)
                            .addChoice("Ultra Low Bass", 1)
                            .addChoice("Low Bass", 2)
                            .addChoice("Less Low Bass", 3)
                            .addChoice("Less Bass Boost", 4)
                            .addChoice("Bass Boost", 5)
                            .addChoice("Ultra Bass Boost", 6));
        }
        ...
        public void performSlashCommand(SlashCommandInteraction event) {
            ...
            switch (id) {
                ...
                default -> { event.replyEmbeds(EmbedUtils.getErrorEmbed("Unknown EQ ID", guildId).build()).queue(); return; }
            }
    ...
    public static class SpeedCommand implements TopLevelSlashCommand {
        ...
        public SlashCommandData getCommandData() {
            return Commands.slash("speed", "Set playback speed")
                    .addOption(OptionType.NUMBER, "factor", "Speed factor (0.1 - 2.0)", true);
        }
    ...
    public static class PitchCommand implements TopLevelSlashCommand {
        ...
        public SlashCommandData getCommandData() {
            return Commands.slash("pitch", "Set playback pitch")
                    .addOption(OptionType.NUMBER, "factor", "Pitch factor (0.1 - 2.0)", true);
        }
    ...
    public static class PlayNextCommand implements TopLevelSlashCommand {
        ...
        @Override
        public SlashCommandData getCommandData() {
            return Commands.slash("playnext", "Play a track next in queue")
                    .addOption(OptionType.STRING, "query", "URL or search query", true);
        }
        ...
        public void performSlashCommand(SlashCommandInteraction event) {
            ...
            event.getHook().sendMessageEmbeds(EmbedUtils.getInfoEmbed("Loading next: " + query, guildId).build()).queue();
        }
    }
    ...
    public static class AddQueueCommand implements TopLevelSlashCommand {
        ...
        @Override
        public SlashCommandData getCommandData() {
            return Commands.slash("addqueue", "Add a track to the queue")
                    .addOption(OptionType.STRING, "query", "URL or search query", true);
        }
        ...
        public void performSlashCommand(SlashCommandInteraction event) {
            ...
            event.getHook().sendMessageEmbeds(EmbedUtils.getInfoEmbed("Searching and adding to queue: " + query, guildId).build()).queue();
        }
    }
    ...
    public static class LyricsCommand implements TopLevelSlashCommand {
        ...
        public void performSlashCommand(SlashCommandInteraction event) {
            ...
            event.deferReply().queue(hook -> {
                LyricsFetcher.fetchAndSendLyrics(link.getNode(), guildId, embed -> hook.sendMessageEmbeds(embed).queue());
            });
        }
    }
}
