...
public class LyricsFetcher {
    ...
    public static void fetchAndSendLyrics(LavalinkNode node, long guildId, Consumer<MessageEmbed> sender) {
        ...
        String baseUri = node.getBaseUri().toString();
        if (baseUri.startsWith("wss://")) baseUri = baseUri.replaceFirst("wss://", "https://");
        else if (baseUri.startsWith("ws://")) baseUri = baseUri.replaceFirst("ws://", "http://");
        String uri = baseUri + "/v4/sessions/" + sessionId + "/players/" + guildId + "/track/lyrics?skipTrackSource=false";
