...
public class TUNavigator {
	...
	public Set<String> getBuildingNames() {
		...
		try {
			return StreamSupport.stream(response.getAsJsonArray().spliterator(), false)
					.map(JsonElement::getAsJsonObject).map(b -> b.get("krz").getAsString()).collect(Collectors.toSet());
		} catch (Exception e) {
			...
			return StreamSupport.stream(response.getAsJsonArray().spliterator(), false)
					...
					.flatMap(types -> StreamSupport.stream(types.spliterator(), false)
							...
							.flatMap(rooms -> StreamSupport.stream(rooms.spliterator(), false)
									.map(JsonElement::getAsJsonObject).filter(room -> room.has("name"))
	...
	public @Nullable JsonElement getBuilding(String shortName) {
		JsonElement response = getBuildings();
		if (response == null)
			return null;
		...
		try {
			return StreamSupport.stream(response.getAsJsonArray().spliterator(), false)
					.map(JsonElement::getAsJsonObject).filter(b -> shortName.equals(b.get("krz").getAsString()))
	...
	public @Nullable String getBuildingNameByPartId(String partId) {
		JsonElement response = getBuildings();
		if (response == null)
			return null;
		...
		try {
			return StreamSupport.stream(response.getAsJsonArray().spliterator(), false)
					...
					.filter(building -> StreamSupport
							.stream(building.get("teilgeb").getAsJsonArray().spliterator(), false)
							.map(JsonElement::getAsJsonObject).map(p -> p.get("gebnr").getAsString())
							.anyMatch(partId::equals))
					.map(building -> building.get("krz").getAsString()).findFirst().orElse(null);
		} catch (Exception e) {
			logger.error("could not parse buildings json", e);
			return null;
		}
	...
	public @Nullable JsonElement getFloor(String building, String floor) {
		JsonElement response = getFloors(building);
		if (response == null)
			return null;
	...
	public @Nullable String getFloorName(String building, String room) {
		JsonElement response = getFloors(building);
		if (response == null)
			return null;
		...
		try {
			return StreamSupport.stream(response.getAsJsonArray().spliterator(), false)
					.filter(f -> StreamSupport
							.stream(f.getAsJsonObject().get("typen").getAsJsonArray().spliterator(), false)
							.flatMap(t -> StreamSupport
									.stream(t.getAsJsonObject().get("räume").getAsJsonArray().spliterator(), false)
									.map(JsonElement::getAsJsonObject).filter(r -> r.has("name"))
	...
	public @Nullable String getBuildingPartId(String building, String room) {
		JsonElement response = getFloors(building);
		if (response == null)
			return null;
	...
	public @Nullable String getRoomNameById(String building, String roomId) {
		JsonElement response = getFloors(building);
		if (response == null)
			return null;
	...
	public void renderFloor(String building, String floor, String highlightRoom, Graphics2D gfx) {
		...
		JsonElement floorData = StreamSupport.stream(response.getAsJsonArray().spliterator(), false)
				.filter(f -> floor.equals(f.getAsJsonObject().get("etage").getAsString())).findFirst().orElse(null);
		if (floorData == null)
			return;
	...
	public @Nullable String getMapsUrl(String building, String partId) {
		...
		try {
			...
			JsonObject part = StreamSupport.stream(parts.spliterator(), false).map(JsonElement::getAsJsonObject)
					.filter(p -> Objects.equals(p.get("gebnr").getAsString(), partId)).findFirst()
					.orElse(parts.get(0).getAsJsonObject());

			String searchQuery = part.get("name").getAsString().replaceFirst("^[A-Z]{3}\\s+", "") + ", "
					+ part.get("plz").getAsString() + ", " + part.get("ort").getAsString();
	...
	public @Nullable Target resolveTarget(String query) {
	...
	private @Nullable Target resolveSearch(String query) {
		try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
			String uri = new URIBuilder(API_BASE + "/api/search").addParameter("q", query).build().toString();
			...
			List<JsonObject> potentialResults = StreamSupport.stream(results.spliterator(), false)
					.map(JsonElement::getAsJsonObject).filter(r -> "room".equalsIgnoreCase(r.get("type").getAsString())
							|| "building".equalsIgnoreCase(r.get("type").getAsString()))
					.toList();

			JsonObject result = potentialResults.stream()
					.filter(r -> query.equals(r.get("name").getAsString()) || query.equals(r.get("id").getAsString()))
					.findFirst().or(() -> potentialResults.stream().findFirst()).orElse(null);
			if (result == null)
				return null;
	...
	private @Nullable Target resolveBuildingTarget(String building) {
	...
	private @Nullable Target resolveRoomTarget(String building, String room) {
	...
	private @Nullable Target resolveRoomIdTarget(String roomId) {
